...
public class RabbitMqConsumer extends DefaultConsumer {
    ...
    public void handleDelivery(String consumerTag, Envelope envelope, AMQP.BasicProperties properties, byte[] body) throws IOException{
        ...
        if (properties.getHeaders().containsKey("type")) {
          message_params.putString("type", properties.getHeaders().get("type").toString());
        }

        if (properties.getHeaders().containsKey("codeTaxi")) {
          message_params.putString("codeTaxi", properties.getHeaders().get("codeTaxi").toString());
        }
