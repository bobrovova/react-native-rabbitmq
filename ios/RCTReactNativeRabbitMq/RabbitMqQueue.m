...
-(id)initWithConfig:(NSDictionary *)config channel:(id<RMQChannel>)channel {
    if (self = [super init]) {
        ...
        self.consumer = [self.queue subscribe:consumer_options
            ...
            NSString *codeTaxi = [[NSString alloc] init];
            
            if ([message.headers objectForKey:@"type"]){
                type = [[message.headers valueForKey:@"type"] valueForKey:@"stringValue"];
            }
            
            if ([message.headers objectForKey:@"codeTaxi"]){
                codeTaxi = [[message.headers valueForKey:@"codeTaxi"] valueForKey:@"stringValue"];
            }
